...
class ActorCritic(nn.Module):
    def __init__(self, action_dim, action_std, env_min_values, env_max_values, n_rollout, n_trajectory, device):
    ...
    def evaluate(self, state_vision, state_proprioception, action):
        ...
        #hidden_intrinsic = self.intrinsic.intrinsic_brims.init_hidden(bsz=self.n_rollout)
        action_mean = torch.zeros(self.n_steps, self.action_dim).to(self.device)
        state_value = torch.zeros(self.n_steps, 1).to(self.device)
        #state_pred = torch.zeros(self.n_steps, 3, 128, 128).to(self.device)
        ...
        for step in range(self.n_trajectory):
            ...
            #action_b = [action[id] for id in range(step, self.n_steps, self.n_trajectory)]
            ...
            # Intrinsic
            #state_pred_b, hidden_intrinsic = self.intrinsic(state_vision_b, state_proprioception_b, action_b, hidden_intrinsic)

            #print(f'state_pred_b: {state_pred_b.shape}')
...
class Intrinsic2(nn.Module):
    def __init__(self, env_min_values, env_max_values, C=64, M=192, in_chan=9, out_chan=3):
        ...
        #self.encoder_vision = ConvNet(9, 128)

        #self.intrinsic_brims = Brims()
    ...
    def forward(self, state_vision, state_proprioception, action):
        ...
        #x, hidden_intrinsic = self.intrinsic_brims(x, hidden_intrinsic)
