...
class BlocksCore(nn.Module):
    ...
    def __init__(self, ninp, nhid, num_blocks_in, num_blocks_out, topkval, step_att, do_gru, num_modules_read_input=2, use_higher=False, higher_separate_att = True):
        ...
        self.inp_att = MultiHeadAttention(n_head=self.inp_heads, d_model_read=self.block_size_out, d_model_write=256, d_model_out=self.att_out, d_k=64, d_v=self.att_out//self.inp_heads, num_blocks_read=num_blocks_out, num_blocks_write=3,residual=False, topk=self.num_blocks_in+1, grad_sparse=False, skip_write=True)
    ...
    def forward(self, inp, hx, cx, idx_layer):
        ...
        if self.use_higher:
            ...
            inp_use = inp_use.reshape((sz_b, 1, self.ninp))
            #print(f'hx: {hx[-1].shape}')
            hx_use = hx[-1].reshape((sz_b, 1, self.ninp))
            ...
            #print('input shape', inp_use.shape)
        ...
        #print(f'inp_use depois do reshape: {inp_use.shape}')
        ...
        #print(f'attention mask: {mask.shape} - mask: {mask}')
        ...
        #print(f'mask após reshape: {mask.shape} -- mask: {mask}')
