...
class ClevrDataset(object):
    ...
    def __init__(self, name):
        ...
        with open(DATASETS[name][ANN_FN]) as f:
    ...
    def get_roidb(
        ...
        nimgs = len(self.scenes) if not cfg.DEBUG else 200
        roidb = [{
            'file_name': self.scenes[i]['image_filename'],
            'height': 320,
            'width': 480,
            'id': self.scenes[i]['image_index']
        } for i in range(nimgs)]
        for entry in roidb:
            self._prep_roidb_entry(entry)
        if gt:
            # Include ground-truth object annotations
            cache_filepath = os.path.join(self.cache_path, self.name+'_gt_roisdb.pkl')
    ...
    def _add_gt_from_cache(self, roidb, cache_filepath):
        ...
        for entry, cached_entry in zip(roidb, cached_roidb):
            ...
            #mask = mask_utils.decode(entry['segms'][0])
            #print(np.shape(mask))
            #exit()
            # To match the original implementation:
            # entry['boxes'] = np.append(
