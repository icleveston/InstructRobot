...
SCENE_FILE = '/home/brain/alana/cog_im/Scenes/scene_NAO_SAC_envV1.9_cuboid.ttt'
...
class NAOBallEnv(gym.Env):
    ...
    def reset(self):
        ...
        if self.pr.running:
            self.epochN += 1
                ...
                #with open(join(self.directory, "rec/","episode_positions.csv"), "a") as csvfile:
            ...
            if self.epochN <= 200:
                ...
                #sim.simSetStringSignal('videoPath', self.directory + '/rec/video{}'.format(self.epochN))
        ...
        if self.init:
            ...
            # Get a random position within the cuboid and set the target position
    ...
    def step(self, action):
        ...
        if not done:
            self.nSteps += 1
        if (done and self.last_let_go) or (self.nSteps >= 250):
            self.episode_positions.append(self.target.get_position().tolist()+
                                           [self.total_reward, self.last_let_go, self.nSteps,
                                           self.epochN, self.time_steped])
        if self.NAO.Ltouch or self.NAO.Rtouch:
            self.episode_positions.append(self.target.get_position().tolist()+
                                           [self.total_reward, self.last_let_go, self.nSteps,
                                           self.epochN, self.time_steped])
