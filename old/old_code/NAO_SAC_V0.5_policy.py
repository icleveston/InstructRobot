...
SCENE_FILE = join(dirname(abspath(__file__)), 'Scenes/scene_NAO_SAC_envV1.7.5_bigBall.ttt')
...
class NAOBallEnv(gym.Env):
    ...
    def step(self, action):
        ...
        lastPreward[8] = -np.sqrt((l_ax - tx)**2 + (l_ay - ty)**2 + (l_az - tz)**2)*100
        reward += lastPreward[8]
        lastPreward[9] = -np.sqrt((r_ax - tx)**2 + (r_ay - ty)**2 + (r_az - tz)**2)*100
        ...
        if not done:
            self.nSteps += 1
        if self.NAO.ObjectRaised:
            self.episode_positions.append([self.ball_position[0], self.ball_position[1],
...
#            print(min(nao.get_joint_positions()[1])< 15*math.pi/180, 
#                  min(nao.get_joint_positions()[3])< 15*math.pi/180)
#            print(sim.simReadCollision(sim.simGetCollisionHandle("CollisionLHand")))
            if o[2]:
                break
        if _ == 1000:
            on = False
