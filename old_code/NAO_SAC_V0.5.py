...
SCENE_FILE = join(dirname(abspath(__file__)), 'Scenes/scene_NAO_SAC_envV1.7.5.ttt')
...
class NAOBallEnv():
    ...
    def step(self, action):
        ...
        reward += -np.sqrt((l_ax - tx)**2 + (l_ay - ty)**2 + (l_az - tz)**2)*1000
        reward += -np.sqrt((r_ax - tx)**2 + (r_ay - ty)**2 + (r_az - tz)**2)*1000

        done = self.NAO.BallFell or self.NAO.ObjectCaught

        self.steped = True

        #Store individual rewards, to
        self.last_episode_reward.append(lastPreward + 
                [-np.sqrt((l_ax - tx)**2 + (l_ay - ty)**2 + (l_az - tz)**2)*1000,
                 -np.sqrt((r_ax - tx)**2 + (r_ay - ty)**2 + (r_az - tz)**2)*1000,
                 reward])
#        [3ftouch, touch, notouch,
#        ObjectCaught,
#        HandPriorlyOpen,
